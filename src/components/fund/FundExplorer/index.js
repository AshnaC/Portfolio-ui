...
class FundExplorer extends React.Component {
    constructor(props) {
        super(props);
        this.state = { compareList: [], showSearchDropDown: true, first: true };
    ...
    showResults = detailsId => () => {
        ...
        if (detailsId) {
            displayList = this.props.fundList.search_results.filter(
                fund => fund.details_id === detailsId
            );
                                ...
                                <div>
                                    {funds.slice(0, 9).map(fund => {
                                        return (
                                            <SearchItem
                                                key={fund.details_id}
                                                onClick={this.showResults(fund.details_id)}>
                                                {fund.name}
                                            </SearchItem>
                                        );
                                    })}
                                    {funds.length > 0 && (
                                        <ShowMore onClick={this.showResults()}>Show More</ShowMore>
                                    )}
                                </div>
                            )}
                        </SearchResultDropDown>
                    ...
                    {this.state.compareList.length > 0 && (
                        <CompareListWrapper>
                            {this.state.compareList.map(fundToCompare => {
                                return (
                                    <CompareItem key={fundToCompare.details_id}>
                                        ...
                                        <CloseLink onClick={this.removeFromCompare(fundToCompare)}>
                                            &times;
                                        </CloseLink>
                                    </CompareItem>
                                );
                            ...
                            {this.state.compareList.length > 1 && (
                                <ShowMore compare onClick={this.onFundsCompare}>
                                    Compare
                                </ShowMore>
                            )}
                        </CompareListWrapper>
